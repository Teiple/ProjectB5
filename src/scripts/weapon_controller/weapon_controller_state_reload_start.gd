...
func enter(msg := {}):
	weapon_controller.play_action("reload_start", 0.0, false)
	max_duration = weapon_controller.get_current_anim_length("reload_start")
	state_start_time = CustomTime.time
	reload_mark = weapon_controller.current_weapon.get_reload_start_mark()
...
func process(delta):
	...
	if time_passed > max_duration:
		_state_machine.transition_to("Base/ReloadMid")
		return
